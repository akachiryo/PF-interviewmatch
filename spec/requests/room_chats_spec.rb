...
RSpec.describe "RoomChats", type: :request do
  describe "POST #create" do
    context "パラメーターが正常な場合" do
      let(:time_tag) { FactoryBot.create :time_tag }
      let(:ocuupation_tag) { FactoryBot.create :ocuupation_tag }
      let(:user) { FactoryBot.create :user }
      let(:room) { FactoryBot.create :room, time_tag_id: time_tag.id, ocuupation_tag_id: ocuupation_tag.id }
